...
from shopdb.models import (Product, Purchase, Replenishment,
...
def _get_balance_between_stocktakings(start, end):
    ...
    out = {"profit": 0, "loss": 0, "balance": 0, "products": {}}
    ...
    for _id in product_ids:
        ...
        # what price.
        res = (
            db.session.query(func.sum(Purchase.amount), func.sum(Purchase.price))
            .filter(Purchase.product_id == _id)
            .filter(Purchase.revoked.is_(False))
            .filter(
                and_(
                    Purchase.timestamp < end.timestamp,
        ...
        res = (
            db.session.query(
                func.sum(Replenishment.amount), func.sum(Replenishment.total_price)
            )
            .join(
            ...
            .filter(
                and_(
                    ReplenishmentCollection.timestamp < end.timestamp,
        ...
        mean_price = _get_product_mean_price_in_time_range(
            _id, start.timestamp, end.timestamp
        )
...
def get_latest_non_revoked_stocktakingcollection() -> StocktakingCollection:
    ...
    """
    return (
        db.session.query(StocktakingCollection)
...
def get_latest_stocktaking_of_product(product_id: int) -> Optional[Stocktaking]:
    ...
    result = (
        db.session.query(Stocktaking)
        .join(
            StocktakingCollection, StocktakingCollection.id == Stocktaking.collection_id
        )
