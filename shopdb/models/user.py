...
from shopdb.exceptions import (CouldNotCreateEntry, NoRemainingAdmin,
                               NothingHasChanged, UserAlreadyVerified,
                               UserNeedsPassword)
...
class User(db.Model):
    ...
    credit = column_property(
        _replenishmentcollection_sum.expression
    ...
    purchases = db.relationship(
        "Purchase", lazy="dynamic", foreign_keys="Purchase.user_id"
    )
    # Link to all deposits of a user.
    deposits = db.relationship(
        "Deposit", lazy="dynamic", foreign_keys="Deposit.user_id"
    )
    # Link to all deposits of a user.
    ...
    def is_admin(self):
        ...
        au = (
            AdminUpdate.query.filter_by(user_id=self.id)
    ...
    def favorites(self):
        ...
        invalid_tag_ids = (
            db.session.query(Tag.id).filter(Tag.is_for_sale.is_(False)).subquery()
        )
        ...
        inactive_product_ids = (
            db.session.query(Product.id).filter(Product.active.is_(False)).subquery()
        )
        ...
        result = (
            ...
            .filter(
                Purchase.product_id.notin_(invalid_product_ids)
            )  # Get only products which are for sale
            .filter(
                Purchase.product_id.notin_(inactive_product_ids)
            )  # Get only products which are active
            .filter(Purchase.revoked.is_(False))  # Get only non revoked purchases
            .order_by(
                func.sum(Purchase.amount).desc()
            )  # Order by the sum of purchase amount
            .all()
        )
