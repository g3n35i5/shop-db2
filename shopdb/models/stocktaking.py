...
class Stocktaking(db.Model):
    ...
    collection_id = db.Column(
        db.Integer, db.ForeignKey("stocktakingcollections.id"), nullable=False
    )
...
class StocktakingCollection(db.Model):
    ...
    stocktakings = db.relationship(
        "Stocktaking", lazy="dynamic", foreign_keys="Stocktaking.collection_id"
    )

    @hybrid_method
    def set_revoked(self, revoked, admin_id):
        sr = StocktakingCollectionRevoke(
            revoked=revoked, admin_id=admin_id, collection_id=self.id
        )
    ...
    def revokehistory(self):
        res = StocktakingCollectionRevoke.query.filter(
            StocktakingCollectionRevoke.collection_id == self.id
        ).all()
        revokehistory = []
        for revoke in res:
