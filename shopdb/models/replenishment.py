...
class Replenishment(db.Model):
    ...
    replcoll_id = db.Column(
        db.Integer, db.ForeignKey("replenishmentcollections.id"), nullable=False
    )
    ...
    def set_revoked(self, revoked, admin_id):
        ...
        non_revoked_replenishments = (
            self.replenishmentcollection.replenishments.filter_by(revoked=False).all()
        )
    ...
    def revokehistory(self):
        res = ReplenishmentRevoke.query.filter(
            ReplenishmentRevoke.repl_id == self.id
        ).all()
        revokehistory = []
        for revoke in res:
...
class ReplenishmentCollection(db.Model):
    ...
    replenishments = db.relationship(
        "Replenishment", lazy="dynamic", foreign_keys="Replenishment.replcoll_id"
    )
    ...
    def set_revoked(self, revoked, admin_id):
        ...
        dr = ReplenishmentCollectionRevoke(
            revoked=revoked, admin_id=admin_id, replcoll_id=self.id
        )
        self.revoked = revoked
        db.session.add(dr)
    ...
    @hybrid_property
    def revokehistory(self):
        res = ReplenishmentCollectionRevoke.query.filter(
            ReplenishmentCollectionRevoke.replcoll_id == self.id
        ).all()
        revokehistory = []
        for revoke in res:
            revokehistory.append(
