...
from shopdb.models import (Deposit, Purchase, ReplenishmentCollection,
...
def get_financial_overview(admin):
    ...
    replcolls = ReplenishmentCollection.query.filter(
        ReplenishmentCollection.revoked.is_(False)
    ).all()
    ...
    pos_pur = sum(
        map(
            abs, list(filter(lambda x: x >= 0, list(map(lambda x: x.price, purchases))))
        )
    )

    pos_dep = sum(
        map(
            abs, list(filter(lambda x: x >= 0, list(map(lambda x: x.amount, deposits))))
        )
    )

    neg_rep = sum(
        map(abs, list(filter(lambda x: x < 0, list(map(lambda x: x.price, replcolls)))))
    )
    ...
    neg_pur = sum(
        map(abs, list(filter(lambda x: x < 0, list(map(lambda x: x.price, purchases)))))
    )

    neg_dep = sum(
        map(abs, list(filter(lambda x: x < 0, list(map(lambda x: x.amount, deposits)))))
    )

    pos_rep = sum(
        map(
            abs, list(filter(lambda x: x >= 0, list(map(lambda x: x.price, replcolls))))
        )
    )
