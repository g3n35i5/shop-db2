...
from shop_db2.helpers.validators import check_allowed_parameters, check_fields_and_types
from shop_db2.models import Product, Stocktaking, StocktakingCollection
...
def create_stocktakingcollections(admin):
    ...
    products = Product.query.filter(Product.active.is_(True)).filter(Product.countable.is_(True)).all()
