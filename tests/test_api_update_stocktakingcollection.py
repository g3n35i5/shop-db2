...
from shopdb.models import StocktakingCollection
...
class UpdateStocktakingCollectionsAPITestCase(BaseAPITestCase):
    def test_revoke_stocktakingcollection(self):
        ...
        res = self.put(
            url="/stocktakingcollections/1", data={"revoked": True}, role="admin"
        )
    ...
    def test_revoke_stocktakingcollection_multiple_times(self):
        ...
        self.assertEqual(res.status_code, 201)
        res = self.put(
            url="/stocktakingcollections/1", data={"revoked": False}, role="admin"
        )
        self.assertEqual(res.status_code, 201)
        res = self.put(
            url="/stocktakingcollections/1", data={"revoked": True}, role="admin"
    ...
    def test_revoke_stocktakingcollection_as_user(self):
        ...
        res = self.put(
            url="/stocktakingcollections/1", data={"revoked": True}, role="user"
        )
    ...
    def test_update_non_existing_stocktakingcollection(self):
        ...
        self.insert_default_stocktakingcollections()
        res = self.put(
            url="/stocktakingcollections/4", data={"revoked": True}, role="admin"
        )
        self.assertEqual(res.status_code, 401)
    ...
    def test_update_stocktakingcollection_unknown_field(self):
        ...
        self.insert_default_stocktakingcollections()
        res = self.put(
            url="/stocktakingcollections/1", data={"Nonsense": ""}, role="admin"
        )
        self.assertEqual(res.status_code, 401)
    ...
    def test_update_stocktakingcollection_wrong_type(self):
        ...
        self.insert_default_stocktakingcollections()
        res = self.put(
            url="/stocktakingcollections/1", data={"revoked": "yes"}, role="admin"
        )
        self.assertEqual(res.status_code, 401)
