...
from shopdb.models import (Deposit, ProductPrice, Purchase,
...
class GetFinancialOverviewAPITestCase(BaseAPITestCase):
    ...
    def test_get_financial_overview(self):
        ...
        p2 = Purchase(
            user_id=2, product_id=2, amount=3, revoked=False, timestamp=t
        )  # <-
        p3 = Purchase(
            user_id=3, product_id=1, amount=2, revoked=False, timestamp=t
        )  # <-
        p4 = Purchase(user_id=1, product_id=2, amount=1, revoked=True, timestamp=t)
        p5 = Purchase(
            user_id=1, product_id=5, amount=1, revoked=False, timestamp=t
        )  # <-
        ...
        d1 = Deposit(
            user_id=1, admin_id=1, comment="Foo", amount=100, revoked=False
        )  # <-
        d2 = Deposit(user_id=2, admin_id=1, comment="Foo", amount=500, revoked=True)
        d3 = Deposit(
            user_id=3, admin_id=1, comment="Foo", amount=300, revoked=False
        )  # <-
        d4 = Deposit(user_id=2, admin_id=1, comment="Foo", amount=200, revoked=True)
        d5 = Deposit(
            user_id=2, admin_id=1, comment="Negative", amount=-100, revoked=False
        )
        ...
        self.assertEqual(
            api_incomes[2]["amount"], negative_replenishmentcollections_price
        )
        ...
        self.assertEqual(api_incomes[2]["name"], "Replenishments")
        self.assertEqual(
            api_incomes[2]["amount"], positive_replenishmentcollections_price
        )
        self.assertEqual(api_incomes[3]["name"], "Stocktakings")
