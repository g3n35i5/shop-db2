...
class QueryParametersAPITestCase(BaseAPITestCase):
    def test_query_parameters_ordering(self):
        ...
        users = json.loads(
            self.get(
                "/users",
                role="admin",
                params={"sort": {"field": "id", "order": "DESC"}},
            ).data
        )
        ...
        self.assertEqual(
            [None, "Bryce", "Daniel", "Mary", "William"],
    ...
    def test_query_parameters_filter(self):
        ...
        users = json.loads(
            self.get(
                "/users", role="admin", params={"filter": {"lastname": "Smith"}}
            ).data
        )
        ...
        users = json.loads(
            self.get(
                "/users", role="admin", params={"filter": {"is_verified": False}}
            ).data
        )
        self.assertEqual(1, len(users))
        ...
        users = json.loads(
            self.get(
                "/users",
                role="admin",
                params={"filter": {"lastname": ["Smith", "Lee"]}},
            ).data
        )
        ...
        users = json.loads(
            self.get("/users", role="admin", params={"filter": {"rank_id": 1}}).data
        )
        ...
        params = {
            "filter": {"lastname": ["Smith", "Lee"], "firstname": "Mary", "id": [2, 3]}
        }
        users = json.loads(self.get("/users", role="admin", params=params).data)
        # There should only be Mary Smith
        self.assertEqual(1, len(users))
        ...
        params = {
            "filter": {"lastname": ["Smith", "Lee"], "firstname": "Mary", "id": 3}
        }
        users = json.loads(self.get("/users", role="admin", params=params).data)
    ...
    def test_query_parameters_sorting(self):
        ...
        users = json.loads(self.get("/users", role="admin", params=params).data)
        self.assertEqual(
            [-1800, -1100, -400, 0, 0], list(map(lambda x: x["credit"], users))
        )
    ...
    def test_query_parameters_pagination(self):
        ...
        users = json.loads(
            self.get(
                "/users", role="admin", params={"pagination": {"page": 1, "perPage": 1}}
            ).data
        )
        ...
        users = json.loads(
            self.get(
                "/users", role="admin", params={"pagination": {"page": 1, "perPage": 3}}
            ).data
        )
        ...
        users = json.loads(
            self.get(
                "/users", role="admin", params={"pagination": {"page": 2, "perPage": 2}}
            ).data
        )
    ...
    def test_invalid_query_parameters(self):
        ...
        param_list = [
            ...
            # Invalid character "!" in filter value
            {
                "sort": {"field": "id", "order": "DESC"},
                "pagination": {"page": 1, "perPage": 1},
                "filter": {"firstname": "Inval!d value"},
            },
            # Little Bobby Tables
            {
                "sort": {"field": "id", "order": "DESC"},
                "pagination": {"page": 1, "perPage": 1},
                "filter": {"firstname": "Robert'); DROP TABLE users;--"},
            },
